...
**Progress-photo uploads.** The iOS app sends original selected-file bytes, including HEIC/HEIF, and the server creates the only JPEG renditions. A new upload replaces the existing photo with the same date and tag. ChatGPT uses `upload_progress_photo_files`, whose `photos` field is declared through `_meta["openai/fileParams"]`; ChatGPT injects temporary attachment download references and the server streams them under the REST 10 MB cap. Other MCP clients can use `upload_progress_photos` with raw base64. Both MCP tools accept up to 30 photos, extract capture metadata, match existing pose tags, and send accepted photos through the same service used by `POST /measures/photos`.

**ChatGPT plugin.** Submit [`dist/pulse-progress-photos-plugin.zip`](dist/pulse-progress-photos-plugin.zip) as the skill package in the same **With MCP** draft as `https://pulse-server-production-4521.up.railway.app/mcp`. The plugin source lives in [`plugins/pulse-progress-photos/`](plugins/pulse-progress-photos/). [`dist/upload-pulse-progress-photos.zip`](dist/upload-pulse-progress-photos.zip) remains the standalone Codex skill artifact and does not connect Pulse by itself.

...
**MCP progress-photo uploads.** ChatGPT uses `upload_progress_photo_files`, whose `photos` field is declared through `_meta["openai/fileParams"]`; ChatGPT injects temporary attachment download references and the server streams them under the REST 10 MB cap. Other MCP clients can use `upload_progress_photos` with raw base64. Both tools accept up to 30 photos, extract capture metadata, match existing pose tags, and send accepted photos through the same service used by `POST /measures/photos`.

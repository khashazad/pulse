...
CLIENT_COOKIE_NAME = "oauth_client"

OAuthClient = Literal["ios", "web"]
...
def _app_redirect(
    *,
    client: OAuthClient = "ios",
    error: str | None = None,
    code: str | None = None,
) -> RedirectResponse:
    """Build a 302 redirect to the configured iOS or web callback.
    ...
    **Inputs:**
    - client ("ios" | "web"): Fixed client destination selected at OAuth start.
    - error (str | None): Error code propagated to the client; ``None`` on success.
    - code (str | None): Opaque one-time exchange code issued on success.

    **Outputs:**
    - RedirectResponse: 302 to the configured client callback with OAuth cookies cleared.
    """
    settings = get_settings()
    base = (
        f"{settings.web_app_url}/login/callback"
        if client == "web" and settings.web_app_url
        else "/login/callback"
        if client == "web"
        else f"{settings.app_redirect_scheme}://auth"
    )
    ...
    response.delete_cookie(CLIENT_COOKIE_NAME, path=STATE_COOKIE_PATH)
    return response


def _oauth_client(cookie_value: str | None) -> OAuthClient:
    """Normalize the untrusted OAuth target cookie to one of two fixed clients.

    **Inputs:**
    - cookie_value (str | None): Target cookie received from the browser callback.

    **Outputs:**
    - OAuthClient: ``"web"`` only for the exact stored marker; otherwise ``"ios"``.
    """
    return "web" if cookie_value == "web" else "ios"
...
async def google_start(
    ...
    **Inputs:**
    ...
    - client ("ios" | "web"): Fixed callback target; defaults to iOS for compatibility.
    ...
    **Raises:**
    ...
    if not code_challenge or method != "S256":
        ...
        return _app_redirect(client=client, error="invalid_request")
...
async def google_callback(
    ...
    **Inputs:**
    ...
    - oauth_client (str | None): Server-set fixed callback target; absent means iOS.
    ...
    **Outputs:**
    ...
    if error:
        ...
        return _app_redirect(client=client, error="access_denied")
    ...
    if not state or not oauth_state or not secrets.compare_digest(state, oauth_state):
        return _app_redirect(client=client, error="invalid_state")
    ...
    if not code:
        return _app_redirect(client=client, error="invalid_callback")
    ...
    try:
        ...
        return _app_redirect(client=client, error="server_error")
    ...
    if email not in settings.allowed_emails_set:
        ...
        return _app_redirect(client=client, error="not_allowed")
    ...
    return _app_redirect(client=client, code=exchange_code)

...
**Photos:** progress photos go through `services/image_processing.py` (`process_progress_photo`) → three JPEG encodings: **archive** (3000 px, q90 preservation copy), **display** (1600 px), and **thumb** (1024 px). The three objects are written to an S3-compatible object store (Cloudflare R2 in prod; `photo_store.py` falls back to a `LocalStore` directory in local dev when S3 env vars are absent), keyed `progress/{user_key}/{photo_id}/{archive|display|thumb}.jpg`, and proxied back through the server — no presigned URLs. Postgres stores only metadata plus `storage_key_prefix`; a temporary bytea fallback handles pre-migration rows that have no prefix. Container photos continue to use `services/image_processing.py` (`process_photo`) and are still stored inline in Postgres. Upload size is capped via `PhotoTooLargeError`.

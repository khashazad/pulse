...
- `server/` — FastAPI + Postgres backend plus a co-deployed React Progress Photos web client. Google-OAuth session/Bearer auth for the apps; MCP endpoint at `/mcp` with GitHub-OAuth + service-token paths. Feature surface: food entries, meals, prep containers, custom foods, food memory, weight, progress photos (+ tags, including MCP bulk upload), USDA proxy. See **Server** below.
- `ios/` — SwiftUI iOS 17+ client (four tabs: Intake, Food, Prep, Measures). Auth is Google sign-in → opaque Bearer session token (stored in Keychain); the base URL is baked in at build time from `PULSE_BASE_URL` (see **iOS → Commands**), and Settings only displays it. The Login/AuthSession flow is the live and only client auth path. See **iOS** below.
...
**Web client (`server/web/`):** React + TypeScript + Vite, built into the server image and also deployed separately to Vercel. Read-only Progress Photos: photo-first timeline (newest captures up top, compact status line with weight delta), multi-select tag chips (persisted locally), calendar modal for capture days (scroll-to-date + compare shortcut), and a redesigned compare view with calendar date pickers, quick presets, and side-by-side pairs filtered by tag. Windowed range loading splits reads into ≤366-day spans. It uses the same Google OAuth session-token flow as iOS; the Vercel build starts OAuth directly on Railway through `VITE_AUTH_ORIGIN`, then Railway returns through `WEB_APP_URL/login/callback`; authenticated data routes are proxied back to Railway by `vercel.json`. Local UI preview: `pnpm dev` + `http://localhost:5173/?preview=1`. Run with Node 24 and pnpm 10.10.0 (`pnpm test --run`, `pnpm typecheck`, `pnpm build`).

**Feature surface** (`server/src/pulse_server/`):

- `routers/` — `auth`, `entries`, `summary`, `targets`, `usda`, `logs`, `containers`, `custom_foods`, `food_memory`, `meals`, `weight`, `measures_photos`, `measures_photo_tags`, `activity`. The `activity` router exposes the read API over the imported Apple/Hevy data: `GET /activity/workouts` (cursor-paginated feed, optional `type` filter), `GET /activity/workouts/{id}` (Apple stats + linked Hevy sets grouped into exercises), `GET /activity/summary` (week/month/year trends — totals, period-over-period deltas, top-5 by-type rollup, volume series, est-1RM top lifts/PRs).
- `services/` — pairs 1:1 with routers for most features; plus `normalize.py`, `image_processing.py` (photo pipeline: `process_photo` for container photos stored inline in Postgres, `process_progress_photo` for progress photos stored in the object store — both invoked via `asyncio.to_thread`, never inline in an async path), `containers_service.py` (container-photo processing + persistence), `progress_photo_service.py` (upload orchestration, object-store writes, cleanup), `alias_utils.py` (alias normalization) and `date_utils.py` (range/date validation; all range reads including `/logs` cap at 366 days), and `rate_limit.py` (lock-free sliding window; applied to the USDA proxy per-user and to `/auth/google/*` per-client-IP). `routers/uploads.py` holds the shared capped multipart read used by both photo upload routers. The activity read API adds `activity_service.py` (feed/detail/summary assembly from `ActivityReadRepository` rows into DTOs) and `activity_summary.py` (pure trend math: Epley est-1RM, week/month/year period bounds, by-type rollup, PR-aware top lifts, volume bucketing — no DB).
- `models/` — Pydantic DTOs (`snake_case`). Mirror these on the iOS side when changing wire format. `models/adapters.py` holds shared row→DTO adapters (`container_response`, `custom_food_response`, `food_memory_entry`, `meal_item_response`, `meal_response`, `meal_summary`, `macro_targets_from_row`) used by both routers and MCP tools. Tiny per-feature modules were merged into `models/daily.py` (logs + summary DTOs + `CaloriesDailyRow`). `models/activity.py` holds the activity read DTOs (workout summary/detail, exercises/sets, trend summary); numeric fields are typed `float` so Pydantic coerces the `Decimal` rows to JSON numbers.
...
**Photos:** progress photos go through `services/image_processing.py` (`process_progress_photo`) → three JPEG encodings: **archive** (3000 px, q90 preservation copy), **display** (1600 px), and **thumb** (1024 px). The server accepts common Pillow formats plus HEIC/HEIF through `pillow-heif`. The iOS progress-photo flow keeps original selected-file bytes and sends them without client JPEG conversion. The three objects are written to an S3-compatible object store (Backblaze B2 in prod (any S3-compatible store works); `photo_store.py` falls back to a `LocalStore` directory in local dev when S3 env vars are absent), keyed `progress/{user_key}/{photo_id}/{archive|display|thumb}.jpg`, and proxied back through the server — no presigned URLs. A new upload replaces all stored rows and objects with the same `(user_key, log_date, tag_id)`. Postgres keeps metadata plus `storage_key_prefix` only (NOT NULL since the object-store cutover completed). Container photos continue to use `services/image_processing.py` (`process_photo`) and are still stored inline in Postgres. Upload size is capped via `PhotoTooLargeError`. MCP offers `upload_progress_photo_files` for ChatGPT's `_meta["openai/fileParams"]` attachment references and `upload_progress_photos` for clients with base64 bytes. Both resolve capture dates and existing pose tags, then delegate accepted images to the same `progress_photo_service.insert_one` path as the REST upload endpoint.
...
Single-user iOS client (SwiftUI, iOS 17+, Swift 5.9) for the self-hosted Pulse backend. Auth is Google sign-in: `Views/Auth/LoginView.swift` + `State/AuthSession.swift` drive an `ASWebAuthenticationSession` OAuth handshake (PKCE) that exchanges for an opaque Bearer session token, stored in Keychain via `KeychainStore`. The base URL is NOT runtime-configurable: it is baked at xcodegen time from `PULSE_BASE_URL` (exported in `ios/.envrc`) into Info.plist, read once by `Config/Constants.swift` (`Constants.baseURL`, which `fatalError`s on a missing/malformed value), and only *displayed* in Settings. Changing servers means re-exporting `PULSE_BASE_URL`, regenerating the project, and rebuilding.
...
- `Config/` — `Constants` (build-time `baseURL` from Info.plist, keychain identifiers, OAuth callback config) and `KeychainStore` (Generic Password item with `kSecAttrAccessibleAfterFirstUnlock`).

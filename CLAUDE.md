...
**Web client (`server/web/`):** React + TypeScript + Vite, co-deployed in the server image and served same-origin by `web.py`. Its scope is read-only Progress Photos: responsive gallery, fullscreen viewer, tag filters, and two-photo comparison with weight deltas. It uses the same Google OAuth session-token flow as iOS; `/auth/google/start?client=web` returns through `WEB_APP_URL/login/callback`. Run with Node 24 and pnpm 10.10.0 (`pnpm test --run`, `pnpm typecheck`, `pnpm build`).

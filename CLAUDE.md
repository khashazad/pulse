...
**Web client (`server/web/`):** React + TypeScript + Vite, built into the server image and also deployed separately to Vercel. Its scope is read-only Progress Photos: responsive gallery, fullscreen viewer, tag filters, and two-photo comparison with weight deltas. It uses the same Google OAuth session-token flow as iOS; the Vercel build starts OAuth directly on Railway through `VITE_AUTH_ORIGIN`, then Railway returns through `WEB_APP_URL/login/callback`; authenticated data routes are proxied back to Railway by `vercel.json`. Run with Node 24 and pnpm 10.10.0 (`pnpm test --run`, `pnpm typecheck`, `pnpm build`).
